...
fn make_etag_is_strong() {
    ...
    let etag =
        make_etag(meta.modified().ok(), meta.len()).expect("modified() is supported on Linux");
    assert!(
        !etag.starts_with("W/"),
        "must be a strong etag (If-Range needs strong comparison): {etag}"
    );
    assert!(
        etag.starts_with('"') && etag.ends_with('"'),
        "must still be a quoted entity-tag: {etag}"
    );
...
fn make_etag_distinguishes_same_second_different_nanosecond() {
    ...
    assert_ne!(
        a, b,
        "same second, same length, different nanosecond mtime must still change the ETag"
    );
}
...
fn not_modified_matches_on_if_none_match() {
    let cond = ConditionalHeaders {
        if_none_match: Some(typed("W/\"abc-1\"")),
        ..Default::default()
    };
    assert!(not_modified(&cond, Some("W/\"abc-1\""), None));
    assert!(
        !not_modified(&cond, Some("W/\"different\""), None),
        "a different etag must not be treated as unmodified"
    );
}
...
fn not_modified_matches_any_tag_in_a_comma_separated_list() {
    ...
    let cond = ConditionalHeaders {
        if_none_match: Some(typed("\"a\", \"b\", \"c\"")),
        ..Default::default()
    };
...
fn not_modified_wildcard_if_none_match_always_matches() {
    let cond = ConditionalHeaders {
        if_none_match: Some(headers::IfNoneMatch::any()),
        ..Default::default()
    };
...
fn if_range_matching_strong_etag_honors_range() {
    let cond = ConditionalHeaders {
        if_range: Some(headers::IfRange::etag(etag("\"x\""))),
        ..Default::default()
    };
...
fn if_range_stale_etag_falls_back_to_full_response() {
    let cond = ConditionalHeaders {
        if_range: Some(headers::IfRange::etag(etag("\"old\""))),
        ..Default::default()
    };
...
fn if_range_rejects_a_weak_client_value_even_when_it_textually_matches() {
    ...
    let cond = ConditionalHeaders {
        if_range: Some(headers::IfRange::etag(etag("W/\"x\""))),
        ..Default::default()
    };
    assert!(!if_range_matches(&cond, Some("\"x\""), None));
}
...
fn if_range_matching_date_honors_range() {
    let modified = SystemTime::UNIX_EPOCH + Duration::from_secs(1_700_000_000);
    let cond = ConditionalHeaders {
        if_range: Some(headers::IfRange::date(modified)),
        ..Default::default()
    };
    assert!(if_range_matches(
        &cond,
