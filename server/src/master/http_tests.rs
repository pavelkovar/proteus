...
fn test_config(routes: Vec<Route>) -> Config {
    Config {
        ...
        php: PhpConfig {
            ...
            limits: Limits { requests: 500, timeout: 30 },
            processes: Processes { max: 4, spare: 2, idle_timeout: 0, spawn_timeout: 30 },
            queue: QueueConfig { max_depth: 0, timeout: 5 },
...
fn static_route(prefix: &str, root: &str) -> Route {
    Route {
        matcher: RouteMatch { uri: vec![MatchPattern::try_from(format!("{prefix}*")).unwrap()], ..Default::default() },
        action: RouteActionConfig::Static { root: root.into(), fallback: None },
    }
}
...
fn matches_static_route_by_prefix() {
    ...
    match match_route(&cfg, "/static/logo.png", "GET", "") {
        RouteDecision::Matched { action: RouteActionConfig::Static { root, fallback } } => {
...
fn falls_through_to_no_match_when_nothing_fits() {
    let cfg = test_config(vec![static_route("/static/", "/var/www/public")]);
    assert_eq!(match_route(&cfg, "/api/whatever", "GET", ""), RouteDecision::NoMatch);
}
...
fn first_matching_route_wins() {
    ...
    match match_route(&cfg, "/uploads/x.jpg", "GET", "") {
        RouteDecision::Matched { action: RouteActionConfig::Static { root, .. } } => {
    ...
    match match_route(&cfg, "/anything-else", "GET", "") {
        RouteDecision::Matched { action: RouteActionConfig::Static { root, fallback } } => {
            ...
            assert_eq!(
                fallback.as_deref(),
                Some(&RouteActionConfig::Php { target: "app".into() })
            );
        }
...
fn php_target_is_carried_into_the_decision() {
    let cfg = test_config(vec![
        Route {
            matcher: RouteMatch { uri: vec![MatchPattern::try_from("/api/*".to_string()).unwrap()], ..Default::default() },
            action: RouteActionConfig::Php { target: "api".into() },
        },
        Route {
            matcher: RouteMatch { uri: vec![MatchPattern::try_from("/legacy/*".to_string()).unwrap()], ..Default::default() },
            action: RouteActionConfig::Static {
                root: "/var/www/legacy".into(),
                fallback: Some(Box::new(RouteActionConfig::Php { target: "legacy".into() })),
            },
        },
    ]);
    ...
    match match_route(&cfg, "/legacy/x.php", "GET", "") {
        RouteDecision::Matched { action: RouteActionConfig::Static { fallback, .. } } => {
            assert_eq!(
                fallback.as_deref(),
                Some(&RouteActionConfig::Php { target: "legacy".into() })
            );
        }
        other => panic!("expected Static, got {other:?}"),
    }
...
fn uri_route(uri_patterns: &[&str], action: RouteActionConfig) -> Route {
    Route {
        matcher: RouteMatch {
            uri: uri_patterns.iter().map(|p| MatchPattern::try_from(p.to_string()).unwrap()).collect(),
            ..Default::default()
        },
        action,
    }
...
fn uri_pattern_glob_wildcard_can_match_zero_characters_at_a_boundary() {
    ...
    assert!(trailing.matches("/admin"), "a trailing wildcard must also match zero extra characters");

    let middle: MatchPattern = "ab*cd".to_string().try_into().unwrap();
    assert!(middle.matches("abcd"), "a middle wildcard must also match zero characters between two literal parts");
    assert!(middle.matches("abXYcd"));
    assert!(!middle.matches("ab"), "shorter than min_length must still be rejected");
}
...
fn uri_pattern_regex_matches_and_rejects_an_invalid_pattern() {
    ...
    assert!(err.contains("invalid match regex"), "unexpected error: {err}");
}
...
fn match_route_supports_a_return_action() {
    let cfg = test_config(vec![uri_route(&["~/\\.", "~^\\."], RouteActionConfig::Return { status: 403 })]);
    match match_route(&cfg, "/.env", "GET", "") {
        RouteDecision::Matched { action: RouteActionConfig::Return { status }, .. } => assert_eq!(*status, 403),
        other => panic!("expected Return, got {other:?}"),
    }
    assert_eq!(match_route(&cfg, "/plain", "GET", ""), RouteDecision::NoMatch);
}
...
fn match_route_combines_negated_and_positive_uri_patterns() {
    ...
    assert!(matches!(match_route(&cfg, "/admin/panel", "GET", ""), RouteDecision::Matched { .. }));
    assert_eq!(match_route(&cfg, "/admin/secret/keys", "GET", ""), RouteDecision::NoMatch);
    assert_eq!(match_route(&cfg, "/other", "GET", ""), RouteDecision::NoMatch);
}
...
fn match_route_treats_a_purely_negative_uri_list_as_p_empty() {
    ...
    let cfg = test_config(vec![uri_route(&["!*.css", "!*.js"], RouteActionConfig::Return { status: 200 })]);
    assert!(matches!(match_route(&cfg, "/index.html", "GET", ""), RouteDecision::Matched { .. }));
    assert_eq!(match_route(&cfg, "/app.js", "GET", ""), RouteDecision::NoMatch);
}
...
fn match_route_treats_a_purely_negative_method_list_as_p_empty() {
    let cfg = test_config(vec![Route {
        matcher: RouteMatch { method: vec!["!OPTIONS".to_string().try_into().unwrap()], ..Default::default() },
        action: RouteActionConfig::Return { status: 200 },
    }]);
    assert!(matches!(match_route(&cfg, "/", "GET", ""), RouteDecision::Matched { .. }));
    assert!(matches!(match_route(&cfg, "/", "POST", ""), RouteDecision::Matched { .. }));
    assert_eq!(match_route(&cfg, "/", "OPTIONS", ""), RouteDecision::NoMatch);
}
...
fn match_route_matches_on_method() {
    let cfg = test_config(vec![Route {
        matcher: RouteMatch {
            method: vec!["GET".to_string().try_into().unwrap(), "HEAD".to_string().try_into().unwrap()],
            ..Default::default()
        },
        action: RouteActionConfig::Return { status: 200 },
    }]);
    assert!(matches!(match_route(&cfg, "/", "GET", ""), RouteDecision::Matched { .. }));
    assert!(matches!(match_route(&cfg, "/", "HEAD", ""), RouteDecision::Matched { .. }));
...
fn match_route_ands_uri_and_method() {
    ...
    assert!(matches!(match_route(&cfg, "/api/widgets", "POST", ""), RouteDecision::Matched { .. }));

    assert_eq!(match_route(&cfg, "/other", "POST", ""), RouteDecision::NoMatch);

    assert_eq!(match_route(&cfg, "/api/widgets", "GET", ""), RouteDecision::NoMatch);
}

#[test]
fn match_route_matches_on_host() {
    let cfg = test_config(vec![Route {
        matcher: RouteMatch { host: vec!["example.test".to_string().try_into().unwrap()], ..Default::default() },
        action: RouteActionConfig::Return { status: 200 },
    }]);
    assert!(matches!(match_route(&cfg, "/", "GET", "example.test"), RouteDecision::Matched { .. }));
    assert_eq!(match_route(&cfg, "/", "GET", "other.test"), RouteDecision::NoMatch);
...
fn match_route_ands_host_with_uri_and_method() {
    ...
    assert!(matches!(match_route(&cfg, "/api/widgets", "GET", "api.example.test"), RouteDecision::Matched { .. }));
    assert_eq!(match_route(&cfg, "/api/widgets", "GET", "other.example.test"), RouteDecision::NoMatch);
}
...
fn server_name_port_handles_bracketed_ipv6_host_with_and_without_a_port() {
    ...
    assert_eq!(resolve_server_name_port(&headers, true, "0.0.0.0:80"), ("[::1]".to_string(), 80));
...
fn https_untrusted_peer_is_ignored_even_with_the_header_set() {
    let mut headers = HeaderMap::new();
    headers.insert("x-forwarded-proto", "https".parse().unwrap());
    assert!(!resolve_https(&headers, false), "an untrusted peer must never be able to spoof HTTPS");
}
...
fn compression_respects_size_threshold() {
    assert_eq!(pick_encoding(100, "gzip", 1024, "text/plain", &[]), None, "below threshold");
    assert_eq!(pick_encoding(2000, "gzip", 1024, "text/plain", &[]), Some(Encoding::Gzip), "above threshold, gzip accepted");
    assert_eq!(pick_encoding(2000, "", 1024, "text/plain", &[]), None, "above threshold but no accept-encoding");
    assert_eq!(pick_encoding(2000, "br", 1024, "text/plain", &[]), Some(Encoding::Brotli), "client wants brotli");
}
...
fn compression_prefers_zstd_then_brotli_then_gzip() {
    ...
    assert_eq!(pick_encoding(2000, "deflate", 1024, "text/plain", &[]), None, "no supported encoding accepted");
}
...
fn compression_respects_weighted_accept_encoding() {
    ...
    assert_eq!(
        pick_encoding(2000, "zstd;q=0, br;q=0.8, gzip;q=0.5", 1024, "text/plain", &[]),
        Some(Encoding::Brotli)
    );
    ...
    assert_eq!(pick_encoding(2000, "*;q=1", 1024, "text/plain", &[]), Some(Encoding::Zstd));
    ...
    assert_eq!(pick_encoding(2000, "*;q=0", 1024, "text/plain", &[]), None, "wildcard veto with no explicit overrides");
}
...
fn framing_headers_from_a_php_script_are_never_forwarded() {
    ...
    assert!(resp.headers().get("content-length").is_none(), "a script-set Content-Length must never reach the client");
    assert!(resp.headers().get("transfer-encoding").is_none(), "a script-set Transfer-Encoding must never reach the client");
    assert!(resp.headers().get("connection").is_none(), "a script-set Connection must never reach the client");
    assert_eq!(resp.headers().get("x-custom").unwrap(), "kept", "non-framing headers must still pass through untouched");
}
...
fn percent_decode_leaves_an_unencoded_path_borrowed() {
    ...
    assert!(matches!(decoded, std::borrow::Cow::Borrowed(_)), "the common case must not allocate");
    assert_eq!(decoded, "/a/b/c.txt");
}

#[test]
fn percent_decode_handles_spaces_and_multibyte() {
    assert_eq!(percent_decode_path("/my%20file.txt").unwrap(), "/my file.txt");
...
fn decoded_traversal_is_caught_by_the_traversal_check() {
    for raw in ["/%2e%2e/etc/passwd", "/a/%2E%2E/%2E%2E/etc/passwd", "/%2e%2e"] {
        ...
        assert!(!path_escapes_root(raw), "sanity: {raw} is exactly the shape the raw check misses");
    }
}
...
fn percent_decode_rejects_encoded_separators() {
    for raw in ["/a%2Fb", "/a%2fb", "/%2e%2e%2fetc/passwd", "/a%5Cb"] {
        assert_eq!(percent_decode_path(raw), Err(PathDecodeError::EncodedSeparator), "{raw} must be refused");
    }
}
...
fn percent_decode_rejects_nul_malformed_and_non_utf8() {
    ...
    assert_eq!(percent_decode_path("/a%zzb"), Err(PathDecodeError::Malformed));
    ...
    assert_eq!(percent_decode_path("/%FF%FE"), Err(PathDecodeError::NotUtf8));
}
...
fn route_matching_cannot_be_dodged_by_encoding_a_character() {
    let cfg = test_config(vec![Route {
        matcher: RouteMatch { uri: vec![MatchPattern::try_from("~\\.php$".to_string()).unwrap()], ..Default::default() },
        action: RouteActionConfig::Php { target: "app".into() },
    }]);
    let raw = "/index%2Ephp";
    let decoded = percent_decode_path(raw).unwrap();

    assert!(
        matches!(match_route(&cfg, &decoded, "GET", ""), RouteDecision::Matched { .. }),
